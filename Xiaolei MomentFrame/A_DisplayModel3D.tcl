proc DisplayModel3D { {ShapeType nill} {dAmp 5}  {xLoc 0} {yLoc 0} {xPixels 0} {yPixels 0} {nEigen 1} } {
	...
	if {  [info exists ScreenResolutionX] != 1} {set ScreenResolutionX 1920};		# set default if it has not been defined previously.
	if {  [info exists ScreenResolutionY] != 1} {set ScreenResolutionY 1080};		# set default if it has not been defined previously.
	...
	if {$ShapeType ==  "DeformedShape" } {
		...
		set viewPlane XY
		set windowTitleTag [format %s%s%s $windowTitle "=>" $viewPlane] 
		recorder display $windowTitleTag $xLoc1 $yLoc $xPixels $yPixels -wipe; # display recorder
		# DisplayPlane $ShapeType $dAmp $viewPlane $nEigen 1
		DisplayPlane $ShapeType $dAmp $viewPlane $nEigen 
		
		set viewPlane ZY
		set windowTitleTag [format %s%s%s $windowTitle "=>" $viewPlane] 
		recorder display $windowTitleTag $xLoc $yLoc $xPixels $yPixels -wipe;
		# DisplayPlane $ShapeType $dAmp $viewPlane $nEigen 2
		DisplayPlane $ShapeType $dAmp $viewPlane $nEigen 
		set viewPlane ZX
		set windowTitleTag [format %s%s%s $windowTitle "=>" $viewPlane] 
		recorder display $windowTitleTag $xLoc $yLoc1 $xPixels $yPixels -wipe;
		# DisplayPlane $ShapeType $dAmp $viewPlane $nEigen 3
		DisplayPlane $ShapeType $dAmp $viewPlane $nEigen 
		set viewPlane 3D
		set windowTitleTag [format %s%s%s $windowTitle "=>" $viewPlane] 
		recorder display $windowTitleTag $xLoc1 $yLoc1 $xPixels $yPixels -wipe;
