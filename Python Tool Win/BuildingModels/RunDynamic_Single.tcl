...
##############################################################################################################################
# Need to modify
set Scale_Sa_GM {0.162958971	0.325917943	0.488876914	0.651835885	0.814794857	0.977753828	1.140712799	1.303671771	1.466630742	1.629589713	1.792548685	1.955507656	2.118466627	2.281425599	2.44438457}; 
set GMset_Num {22 22 22 22 22 22 22 22 22 22 22 22 22 22 22}; #list contains the number of ground motions in each hazard levle
...
for {set i 0} {$i <= $Model_Num-1} {incr i} {
	...
	set pathToTextFile $baseDir/GM_sets/FEMA44_FarFault/$ScaleID1/GroundMotionInfo;
	...
	set GM_XFileName [format %s%s%s%s%s%s $baseDir /GM_sets/FEMA44_FarFault/ $ScaleID1 /histories/ [lindex $groundMotionFileNames [expr $GM_XNumber-1]] .txt];
	set GM_ZFileName [format %s%s%s%s%s%s $baseDir /GM_sets/FEMA44_FarFault/ $ScaleID1 /histories/ [lindex $groundMotionFileNames [expr $GM_ZNumber-1]] .txt];
	...
	set pathToModel $baseDir/Building_T1-60x30-S2-GS_Existing/DynamicAnalysis;
	cd	$pathToModel
